...
use_pdb_only_or_pdb_and_xyz = 'pdb_and_xyz' # choose 'pdb_only' or 'pdb_and_xyz'

filepath_list = ['../set1/1r1', '../set1/1r2', '../set1/1r3', '../set1/1r4', '../set1/1r5']  #without slash after
pore_center = (20 + 10) / 10

area_of_slit = 29.472 * 29.777
...
for i in range(len(filepath_list)):
    ...
    if use_pdb_only_or_pdb_and_xyz == 'pdb_and_xyz':
        #use these 3 lines below and run with xyz file to speed up analsyis and not consume > 200GB memory.
        # this analysis will be an estimated 10x faster
        xyz_filename = filepath + '/' + 'SPCE_PORE_NVT_20_BOX_0_water_only.xyz'
        pdb_filename = filepath + '/' + 'SPCE_PORE_NVT_20_BOX_0_at_0M_steps_water_only.pdb'
        traj = md.load(xyz_filename, top=pdb_filename)[int(10 * 10 ** 6 / 1000):]
    elif use_pdb_only_or_pdb_and_xyz == 'pdb_only':
        # use these lines below to use the raw GOMC pdb files for analysis, but is will consume > 200GB memory.
        # this analysis will be an estimated 10x slower than using the 'pdb_and_xyz' method
        xyz_filename = filepath + '/' + 'SPCE_PORE_NVT_20_BOX_0.pdb'
        pdb_filename = filepath + '/' + 'SPCE_PORE_NVT_20_BOX_0.pdb'
        traj = md.load( xyz_filename)[int(10 * 10 ** 6 / 1000):]
    else:
        print("ERROR: neither use_pdb_only_or_pdb_and_xyz = 'pdb_only' or 'pdb_and_xyz'")

    print('orig traj :' + str(traj))
