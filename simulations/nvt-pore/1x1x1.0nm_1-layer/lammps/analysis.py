...
def number_density(job):
    ...
    h_densities = list()
    fig, ax = plt.subplots()
    for trj in md.iterload(os.path.join(job.ws, 'nvt.dcd'), top=os.path.join(job.ws, 'init.gro'), chunk=1000, skip=2001):
...
def s_order(job):
    ...
    fig, ax = plt.subplots()
    s_list = list()
    for trj in md.iterload(os.path.join(job.ws, 'nvt.dcd'), top=os.path.join(job.ws, 'init.mol2'), chunk=4000, skip=2001):
